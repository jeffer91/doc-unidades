...
export default function App(){
  ...
  const sections=useMemo(()=>[coverSection,...document.sections],[document]);
  const [sectionId,setSectionId]=useState('PORTADA'); const section=sections.find(s=>s.id===sectionId) ?? sections[0];
  useEffect(()=>{window.docUnits.periods.list().then(setPeriods)},[]);
  useEffect(()=>{const p=registry[unit][0];setProcessId(p.id);setDocumentId(p.documents[0].id);setSectionId('PORTADA')},[unit]);
  useEffect(()=>{const p=registry[unit].find(x=>x.id===processId)??registry[unit][0];const d=p.documents[0];setDocumentId(d.id);setSectionId('PORTADA')},[processId]);
  useEffect(()=>{setSectionId('PORTADA')},[documentId]);
