...
export function TextSection({section,ctx,designMode}:{section:SectionConfig;ctx:any;designMode:boolean}){
  ...
  return <div className="work-card"><div className="section-heading"><div><h2>{section.label}</h2><p>Texto editable del período. Se guarda automáticamente; la plantilla base se modifica desde Diseño.</p></div><span className="save-state">{status}</span></div><textarea className="editor tall" value={content} onChange={e=>setContent(e.target.value)}/><div className="action-row"><button className="secondary" onClick={()=>setContent(savedTemplate)}>Usar plantilla actual</button><button onClick={saveContent}>Guardar ahora</button></div><div className="preview-panel"><div className="preview-label">Vista previa</div><p>{preview}</p></div></div>
}
