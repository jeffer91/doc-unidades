...
export function SummarySection({ctx,onOpenSection}:Props){
  ...
  async function loadAll(){
    ...
    const entries=await Promise.all(items.map(async item=>{const rows=await rowsFor(item.matrixId);return [item.id,{rows,errors:validateRows(item,rows)}] as const;}));
  ...
  async function onFile(file:File){
    ...
    const wb=XLSX.read(await file.arrayBuffer());const ws=wb.Sheets[wb.SheetNames[0]];const rows=XLSX.utils.sheet_to_json(ws,{defval:''}) as any[];const errors=validateRows(uploadItem,rows);
  ...
  async function generatePdf(draft:boolean){
    ...
    try{
      ...
      if(!draft&&fieldPending.length){setMessage(`PDF final bloqueado: ${fieldPending.slice(0,3).join(' · ')}${fieldPending.length>3?' …':''}`);return;}
