...
export function SummarySection({ctx,onOpenSection}:Props){
  ...
  return <>
    <div className="work-card summary-card">
      ...
      <div className="document-actions"><div><strong>Documento</strong><span>{allComplete?'Listo para PDF final':`${pending.length} matriz${pending.length===1?'':'ces'} pendiente${pending.length===1?'':'s'} · ${dynamicFieldPending.length} campo${dynamicFieldPending.length===1?'':'s'} pendiente${dynamicFieldPending.length===1?'':'s'}`}</span></div><div className="document-buttons"><button className="secondary" disabled={generating} onClick={()=>generatePdf(true)}>Descargar borrador PDF</button><button disabled={!allComplete||generating} title={!allComplete?'Corrige matrices y campos automáticos pendientes para habilitar el PDF final':''} onClick={()=>generatePdf(false)}>Generar PDF final</button></div></div>
